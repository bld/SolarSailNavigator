...
namespace SolarSailNavigator {
	...
	// Constructor
	public Frame (string[] names, float[] defaults, Qbase qbasefn, Qlocal qlocalfn, Q qfn) {
	    this.names = names;
	...
	public static Dictionary<string, Frame> Frames = new Dictionary<string, Frame>
		...
		{"RTN", new Frame(new string[] {"Cone", "Clock", "Flatspin"},
		...
		{"ICN", new Frame(new string[] {"FPA", "Az", "Flatspin"},
				  new float[] {0f, 0f, 0f},
				  ICNFrame,
				  FAFLocal,
				  FAFFrame)}
	...
	public static Quaternion SailFrame (Orbit orbit, double UT, float[] angles) {
	    var QRTN = RTNFrame(orbit, UT);
	    var QSL = SailFrameLocal(angles[0], angles[1], angles[2]);
	    return QRTN * QSL;
	}
    }
}
